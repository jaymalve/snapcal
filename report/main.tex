...
Reliable meal understanding matters in industry because manual nutrition logging is slow, inconsistent, and difficult to scale. Consumer health apps, telehealth platforms, digital coaching products, and wellness programs all benefit from photo-based meal estimation that reduces user effort while keeping nutrition feedback fast and accessible. Restaurants, meal-delivery services, and smart kitchen products can also use food recognition to support nutrition labeling, portion guidance, and personalized recommendations. For these settings, the challenge is not just classifying clean benchmark images, but producing dependable estimates from messy real-world photos where background clutter can degrade accuracy.
...
\section{Discussion}
Several aspects of the project worked well. The repository matured into a complete end-to-end system that covered preprocessing, training, evaluation, deployment, and frontend interaction in one reproducible codebase. The verified ResNet-50 raw baseline also confirmed that the core learning pipeline was effective, while the deployed stack extended the project beyond a single-model prototype by supporting live model selection and optional SAM preprocessing.

The project also exposed clear limitations. The main research limitation was compute scale: the controlled segmented experiment remained a reduced-manifest ResNet-50 pilot rather than a full cross-architecture study. ViT-B/16 also underperformed under the short local training schedule, showing that fair backbone comparison depended heavily on training budget and tuning. In addition, calorie evaluation remained incomplete because the Food-101-to-USDA mapping still needed curation and the external evaluation protocol was not yet finalized.

Overall, SnapCal succeeded more clearly as a deployed engineering system than as a fully completed experimental study. The strongest next steps are scaling the segmentation experiments, revisiting weaker backbones under better training settings, and formalizing calorie benchmarking.

\section{Conclusion and Future Work}
SnapCal delivered an end-to-end food recognition and calorie estimation system that combined Food-101 classification, optional MobileSAM preprocessing, USDA nutrition lookup, and a deployed web inference workflow. The final system supported live model selection across ResNet-50, EfficientNet-B0, and ViT-B/16, plus portion-scaled nutrition output through a Django-backed API.

The strongest verified model was the raw-image ResNet-50 baseline at 84.83\% top-1 and 97.00\% top-5 accuracy on the full Food-101 test split. EfficientNet-B0 performed similarly, while ViT-B/16 lagged under the same short local training setup. A reduced-manifest ResNet-50 pilot also provided an early segmentation result, where segmented inputs underperformed raw inputs, but that finding remains preliminary because of the small evaluation set and speed-oriented local preprocessing configuration.

Future work should focus on full-scale segmented experiments across all supported architectures, stronger tuning for weaker backbones, and a formal calorie-evaluation protocol after USDA mapping curation is complete.
...
\noindent \textbf{Frontend:} \url{https://snapcal.jaymalve.dev}

\noindent \textbf{Django API:} \url{https://api.snapcal.jaymalve.dev}

\noindent \textbf{GitHub repository:} \url{https://github.com/jaymalve/snapcal}

\noindent \textbf{Modal ResNet-50 Health Endpoint:} \url{https://jaymalveus--snapcal-inference-health-resnet50.modal.run}

\noindent \textbf{Modal ResNet-50 Prediction Endpoint:} \url{https://jaymalveus--snapcal-inference-predict-resnet50.modal.run}

\noindent \textbf{Modal EfficientNet-B0 Health Endpoint:} \url{https://jaymalveus--snapcal-inference-health-efficientnet-b0.modal.run}

\noindent \textbf{Modal EfficientNet-B0 Prediction Endpoint:} \url{https://jaymalveus--snapcal-inference-predict-efficientnet-b0.modal.run}

\noindent \textbf{Modal ViT-B/16 Health Endpoint:} \url{https://jaymalveus--snapcal-inference-health-vit-b16.modal.run}

\noindent \textbf{Modal ViT-B/16 Prediction Endpoint:} \url{https://jaymalveus--snapcal-inference-predict-vit-b16.modal.run}
