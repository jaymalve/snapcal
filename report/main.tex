...
\begin{table}
  \caption{Training configuration summary. The base configuration reflects the general project setup, the verified local baseline reflects the strongest full-dataset run in the repository, and the pilot column reflects the matched quick ablation used for the preliminary segmentation study.}
  \label{tab:training-config}
  \begin{tabular}{lccc}
    \toprule
    Setting & Base pipeline & Verified local baseline & Quick pilot ablation \\
    \midrule
...
\begin{figure}
  ...
  \caption{High-level SnapCal pipeline. A future revision can replace this placeholder with a polished architecture diagram.}
...
\begin{table}
  ...
  \begin{tabular}{lp{0.5\linewidth}}
    ...
    Portion-scaling evaluation note & Implemented in the product workflow, but not yet benchmarked against external calorie labels \\
    \bottomrule
  \end{tabular}
\end{table}
...
\item paired pilot reports for `resnet50\_raw\_quick\_ablation` and `resnet50\_segmented\_quick\_ablation`
